    ...
    quote do
      ...
      def rpc(command, timeout \\ @timeout) do
        try do
          GenServer.call(
            unquote(name),
            {{:call, command}, :erlang.monotonic_time(:milli_seconds) + timeout},
            timeout
          )
        catch
          :exit, {:timeout, _} ->
      ...
      defp rabbitmq_connect() do
        case Connection.open(get_connection_string()) do
          {:ok, conn} ->
            ...
            :erlang.send_after(unquote(cleanup_interval), :erlang.self(), :cleanup_trigger)
            %{channel: chan, reply_queue: reply_queue, correlation_id: 0, continuations: %{}}
      ...
      defp cleanup_timedout_continuations(state) do
        if state != :not_connected and state != nil do
          ...
          new_continuations =
            clist |> Enum.reject(fn {_, {_, timeout}} -> continuation_timed_out(timeout) end)
